    ...
    public JsonObject getEditionStatistiqueCAparTypeclient(String dateBegin, String dateEnd, Locale locale) throws ParseException {
        ...
        if (!dateBegin.equals("") && !dateEnd.equals("")) {
            ...
            if (!listVPmsCa.isEmpty()) {
                ...
                HashMap<String, TreeMap> resultat = new HashMap<>(); // groupement par typeclient
                TreeMap<String, TreeMap> resultatSorted = new TreeMap<>(); // groupement par typeclient trié
                TreeMap<String, BigDecimal> totalVerticalParMois = new TreeMap<>();
                for(int m = 0 ; m < moisAparcourir.size(); m++) {
                    totalVerticalParMois.put(moisAparcourir.get(m), BigDecimal.ZERO);
                }
                ...
                for (int i = 0; i < listVPmsCa.size(); i++) {
                    ...
                    if (!resultat.containsKey(previousTypeClient)) {
                        ...
                        resultat.put(previousTypeClient, tmp);
                    }
                    ...
                    if (sameMonth) {
                        if (sameTypeClient) {
                            ...
                            TreeMap<String, BigDecimal> listeCAtypeClient = resultat.get(previousTypeClient);
                            BigDecimal lastCAtypeClient = listeCAtypeClient.get(previousDateCAformatted);
                            listeCAtypeClient.replace(previousDateCAformatted, lastCAtypeClient.add(totalCAtypeClient));
                        ...
                        previousDateCAformatted = previousDateCA.format(dateTimeFormatter);
                        moisIndex++;
                        listeCAtypeClient = resultat.get(previousTypeClient);
                        lastCAtypeClient = listeCAtypeClient.get(previousDateCAformatted);
                        listeCAtypeClient.replace(previousDateCAformatted, lastCAtypeClient.add(totalCAtypeClient));
                    }
                ...
                for (Map.Entry<String, TreeMap> entry : resultatSorted.entrySet()) {
                    String ligne = "<tr><td class=\"text-center\">" + entry.getKey() + "</td>";
                    TreeMap<String, BigDecimal> listCAtypeClient = entry.getValue();
                    for (Map.Entry<String, BigDecimal> entri : listCAtypeClient.entrySet()) {
                        ligne += "<td class=\"text-center\">" + entri.getValue().toString() + "</td>";
                        totalVerticalParMois.replace(entri.getKey(), totalVerticalParMois.get(entri.getKey()).add(entri.getValue()));
                    }
                    if (nbMois > 1) {
