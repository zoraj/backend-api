...
public class ClosureDao {
    ...
    public void setPosClotureDefinitive(JsonObject request)
        ...
        var closureCashing = getAllListCashing();
        var closureValBacMois = getAllValBacMois();
        var closureValBacAnnees = getAllValBacAnnees();
        var closureJsonObject = Json.createObjectBuilder()
                ...
                .add("closureCashing", closureCashing)
                .add("closureValBacMois", closureValBacMois)
                .add("closureValBacAnnees", closureValBacAnnees)
                .build();
        return closureJsonObject;
    }
            ...
            for(VCollectiviteLecturePrestation caList : listCa){
                if(idInit.equals(caList.getId())){
                    ...
                    var caJson = Json.createObjectBuilder()
                        ...
                        .build();
                    caResults.add(caJson);                    
                    montantTtc = caList.getMontantTtc();
                    montantHt = caList.getMontantHt();
                    quantite = caList.getQte();                   
                    libelle = caList.getLibellePrestation();                   
                    idInit = caList.getId();
                    txTva = caList.getTxTva();
        ...
        for (Object[] n : nbNature) {
                ...
                if (n.length > 1) {
                    ...
                    var rowJson = Json.createObjectBuilder()
                            ...
                            .add("nbPassageJour", new BigDecimal(n[10].toString()))
                            .build();
    ...
    public JsonArray getAllValBacMois() {
        var jsonArray = Json.createArrayBuilder();
        TMmcParametrage valueDateLogicielle = entityManager.find(TMmcParametrage.class, "DATE_LOGICIELLE");
        LocalDate dateLogicielle = LocalDate.parse(valueDateLogicielle.getValeur());
        System.out.println("dateLogicielle"+dateLogicielle);
        Month month = dateLogicielle.getMonth();
        int monthAsInt = month.getValue();
        System.out.println("monthAsInt"+monthAsInt);
        List<Object[]> nbNature = entityManager.createNativeQuery("CALL p_val_bac_mois(:monthAsInt) ")
                .setParameter("monthAsInt", monthAsInt).getResultList();
        for (Object[] n : nbNature) {
                ...
                if (n.length > 1) {
                    ...
                    var rowJson = Json.createObjectBuilder()
                            ...
                            .add("debitLiquide", new BigDecimal(n[0].toString()))
                            .add("subvention", new BigDecimal(n[1].toString()))
                            .add("admission", new BigDecimal(n[2].toString()))
                            .add("cotisation", new BigDecimal(n[3].toString()))
                            .add("debitSolide", new BigDecimal(n[4].toString()))
                            .add("nbrClient", new BigDecimal(n[5].toString()))
                            .add("nbPassageJour", new BigDecimal(n[6].toString()))
                            .build();
                    jsonArray.add(rowJson);
                }  
        }
    ...
    public JsonArray getAllValBacAnnees() {
        var jsonArray = Json.createArrayBuilder();
        TMmcParametrage valueDateLogicielle = entityManager.find(TMmcParametrage.class, "DATE_LOGICIELLE");
        LocalDate dateLogicielle = LocalDate.parse(valueDateLogicielle.getValeur());
        int year = dateLogicielle.getYear();
        List<Object[]> nbNature = entityManager.createNativeQuery("CALL p_val_bac_annees(:year) ")
